...
def compute_slide_stats(rgb: np.ndarray, cfg: PatchSelectionConfig) -> SlideStats:
    ...
    tissue = build_tissue_mask(od_sum, sat, value, cfg)
    tissue_mask = tissue if tissue.any() else np.ones_like(tissue, dtype=bool)
    return SlideStats(
